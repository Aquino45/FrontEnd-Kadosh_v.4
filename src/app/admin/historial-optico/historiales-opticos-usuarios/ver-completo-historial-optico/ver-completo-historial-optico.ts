...
export class VerCompletoHistorialOpticoComponent implements OnInit {
  ...
  private buildFechaIso(fechaStr: string): string {
    // Si no hay fecha, no mandamos nada
    if (!fechaStr) return '';
    
    // 1. Agarramos el valor del input (yyyy-mm-dd)
    const [y, m, d] = fechaStr.split('-');
    
    // 2. 🚨 LA CLAVE: Recuperamos la hora exacta que ya tenía el historial original
    // this.historial.fecha tiene algo como "05/12/2025 14:30:15"
    // .split(' ')[1] agarra solo el "14:30:15"
    const horaOriginal = this.historial.fecha.split(' ')[1]; 

    // 3. Mandamos la misma fecha con su misma hora original al servidor
    return `${d}/${m}/${y} ${horaOriginal}`;
  }
